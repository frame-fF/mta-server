...
addCommandHandler("lean",
	function(player)
		if (not animEnable[player]) then
			...
			triggerClientEvent(syncPlayers, "lean", player, true)
			outputChatBox("[!]#FFFFFF Lean yapıyorsun.", player, 0, 255, 0, true)
		else
			animEnable[player] = false
			triggerClientEvent(syncPlayers, "lean", player, false)
			outputChatBox("[!]#FFFFFF Artık Lean yapmıyorsun.", player, 255, 0, 0, true)
		end
	end
)
...
addEventHandler("onClientSync", resourceRoot,
    function()
		...
		for player, enable in ipairs(animEnable) do
			if (enable) then
				triggerClientEvent(client, "lean", player, true)
			end
		end
    end 
)
